...
This document is a slightly modified copy of [rust-api-guidelines/README.md](https://github.com/brson/rust-api-guidelines/blob/235cfddb62ad84f967c4d484948fa0ccacb7182a/README.md).
...
- **Naming** *(crate aligns with Rust naming conventions)*
  ...
  - [y] Methods on collections that produce iterators follow `iter`, `iter_mut`, `into_iter` ([C-ITER])
  - [y] Iterator type names match the methods that produce them ([C-ITER-TY])
  - [y] Ownership suffixes use `_mut` and `_ref` ([C-OWN-SUFFIX])
  - [y] Single-element containers implement appropriate getters ([C-GETTERS])
- **Interoperability** *(crate interacts nicely with other library functionality)*
  - [y] Types eagerly implement common traits ([C-COMMON-TRAITS])
    - `Copy`, `Clone`, `Eq`, `PartialEq`, `Ord`, `PartialOrd`, `Hash`, `Debug`,
      `Display`, `Default`
  - [y] Conversions use the standard traits `From`, `AsRef`, `AsMut` ([C-CONV-TRAITS])
  - [y] Collections implement `FromIterator` and `Extend` ([C-COLLECT])
  ...
  - [y] Types are `Send` and `Sync` where possible ([C-SEND-SYNC])
  - [y] Error types are `Send` and `Sync` ([C-SEND-SYNC-ERR])
  - [y] Error types are meaningful, not `()` ([C-MEANINGFUL-ERR])
  - [y] Binary number types provide `Hex`, `Octal`, `Binary` formatting ([C-NUM-FMT])
- **Macros** *(crate presents well-behaved macros)*
  - [y] Input syntax is evocative of the output ([C-EVOCATIVE])
  - [y] Macros compose well with attributes ([C-MACRO-ATTR])
  - [y] Item macros work anywhere that items are allowed ([C-ANYWHERE])
  - [y] Item macros support visibility specifiers ([C-MACRO-VIS])
  - [y] Type fragments are flexible ([C-MACRO-TY])
- **Documentation** *(crate is abundantly documented)*
  - [ ] Crate level docs are thorough and include examples ([C-CRATE-DOC])
  - [ ] All items have a rustdoc example ([C-EXAMPLE])
  - [ ] Examples use `?`, not `try!`, not `unwrap` ([C-QUESTION-MARK])
  - [y] Function docs include error conditions in "Errors" section ([C-ERROR-DOC])
  - [y] Function docs include panic conditions in "Panics" section ([C-PANIC-DOC])
  - [y] Prose contains hyperlinks to relevant things ([C-LINK])
  - [y] Cargo.toml publishes CI badges for tier 1 platforms ([C-CI])
  - [y] Cargo.toml includes all common metadata ([C-METADATA])
    - authors, description, license, homepage, documentation, repository,
      readme, keywords, categories
  - [y] Crate sets html_root_url attribute "https://docs.rs/$crate/$version" ([C-HTML-ROOT])
  - [y] Cargo.toml documentation key points to "https://docs.rs/$crate" ([C-DOCS-RS])
  - [ ] Release notes document all significant changes ([C-RELNOTES])
- **Predictability** *(crate enables legible code that acts how it looks)*
  - [y] Smart pointers do not add inherent methods ([C-SMART-PTR])
  ...
  - [y] Operator overloads are unsurprising ([C-OVERLOAD])
  - [y] Only smart pointers implement `Deref` and `DerefMut` ([C-DEREF])
  - [y] `Deref` and `DerefMut` never fail ([C-DEREF-FAIL])
  - [y] Constructors are static, inherent methods ([C-CTOR])
...
- **Type safety** *(crate leverages the type system effectively)*
  ...
  - [y] Arguments convey meaning through types, not `bool` or `Option` ([C-CUSTOM-TYPE])
  - [y] Types for a set of flags are `bitflags`, not enums ([C-BITFLAG])
  - [y] Builders enable construction of complex values ([C-BUILDER])
- **Dependability** *(crate is unlikely to do the wrong thing)*
  - [y] Functions validate their arguments ([C-VALIDATE])
  - [y] Destructors never fail ([C-DTOR-FAIL])
  - [y] Destructors that may block have alternatives ([C-DTOR-BLOCK])
- **Debuggability** *(crate is conducive to easy debugging)*
  - [y] All public types implement `Debug` ([C-DEBUG])
  - [y] `Debug` representation is never empty ([C-DEBUG-NONEMPTY])
- **Future proofing** *(crate is free to improve without breaking users' code)*
  - [y] Structs have private fields ([C-STRUCT-PRIVATE])
  - [y] Newtypes encapsulate implementation details ([C-NEWTYPE-HIDE])
- **Necessities** *(to whom they matter, they really matter)*
  - [y] Public dependencies of a stable crate are stable ([C-STABLE])
 ...
 * Apache License, Version 2.0, ([LICENSE-APACHE](https://github.com/brson/rust-api-guidelines/blob/master/LICENSE-APACHE) or
   http://www.apache.org/licenses/LICENSE-2.0)
 * MIT license ([LICENSE-MIT](https://github.com/brson/rust-api-guidelines/blob/master/LICENSE-MIT) or
